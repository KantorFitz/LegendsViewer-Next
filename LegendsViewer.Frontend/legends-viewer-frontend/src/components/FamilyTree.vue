<template>
    <div v-if="fullscreen == false" ref="cy" style="width: 100%; height: 360px;"></div>
    <div v-else ref="cy" style="width: 100%; height: 720px;"></div>
</template>

<script lang="ts">
import { defineComponent, onMounted, ref } from 'vue';
import cytoscape from 'cytoscape';
import dagre from 'cytoscape-dagre';

cytoscape.use(dagre);

export const leaderImageData = 'data:image/png;base64,)';
export const necromancerImageData = 'data:image/png;base64,';
export const vampireImageData = 'data:image/png;base64,';
export const werebeastImageData = 'data:image/png;base64,';
export const ghostImageData = 'data:image/png;base64,';


export default defineComponent({
    name: 'FamilyTree',
    props: {
        data: {
            type: Object,
            required: true,
        },
        fullscreen: {
            type: Boolean,
            default: false
        }
    },
    setup(props) {
        const cy = ref<HTMLDivElement | null>(null);

        onMounted(() => {
            if (cy.value) {
                const cyInstance = cytoscape({
                    container: cy.value,
                    zoom: .5,
                    wheelSensitivity: 0.1,
                    elements: {
                        nodes: props.data.nodes,
                        edges: props.data.edges
                    }, // Family tree data will be passed as prop
                    style: [
                        {
                            selector: 'node',
                            style: {
                                'shape': 'roundrectangle',
                                'width': 'label',
                                'height': 'label',
                                'content': 'data(label)',
                                'text-wrap': 'wrap',
                                'font-family': 'Helvetica',
                                'font-size': 12,
                                'text-valign': 'center',
                                'color': '#eee',
                                "padding-top": '6px'
                            },
                        },
                        {
                            selector: 'node.current',
                            style: {
                                'background-blacken': 0.1,
                                'border-width': '2px',
                                'border-style': 'dashed',
                                'border-color': 'orange'
                            },
                        },
                        {
                            selector: 'node.dead',
                            style: {
                                'background-opacity': 0.3,
                                'border-opacity': 0.3,
                            },
                        },
                        {
                            selector: 'node.male',
                            style: {
                                'background-color': 'blue',
                            },
                        },
                        {
                            selector: 'node.female',
                            style: {
                                'background-color': 'magenta',
                            },
                        },
                        {
                            selector: 'node.leader',
                            style: {
                                'shape': 'round-octagon',
                                'background-image': 'url(' + leaderImageData + ')',
                                'background-width': '24px',
                                'background-height': '24px',
                                'background-position-y': '2px',
                                'padding-left': '30px',
                            },
                        },
                        {
                            selector: 'node.necromancer',
                            style: {
                                'shape': 'round-hexagon',
                                'background-blacken': 0.1,
                                'background-image': 'url(' + necromancerImageData + ')',
                                'background-width': '24px',
                                'background-height': '24px',
                                'background-position-y': '2px',
                                'padding-left': '36px',
                            },
                        },
                        {
                            selector: 'node.vampire',
                            style: {
                                'shape': 'hexagon',
                                'background-blacken': 0.2,
                                'background-image': 'url(' + vampireImageData + ')',
                                'background-width': '24px',
                                'background-height': '24px',
                                'background-position-y': '2px',
                                'padding-left': '36px',
                            },
                        },
                        {
                            selector: 'node.werebeast',
                            style: {
                                'shape': 'hexagon',
                                'background-blacken': 0.1,
                                'background-image': 'url(' + werebeastImageData + ')',
                                'background-width': '24px',
                                'background-height': '24px',
                                'background-position-y': '2px',
                                'padding-left': '36px',
                            },
                        },
                        {
                            selector: 'node.ghost',
                            style: {
                                'shape': 'hexagon',
                                'background-blacken': -0.1,
                                'background-image': 'url(' + ghostImageData + ')',
                                'background-width': '24px',
                                'background-height': '24px',
                                'background-position-y': '2px',
                                'padding-left': '36px',
                            },
                        },
                        {
                            selector: 'edge',
                            style: {
                                'width': '2px',
                                'line-color': '#666',
                                'curve-style': 'straight'
                            },
                        },
                    ],
                    layout: {
                        name: 'dagre',
                    },
                });

                cyInstance.center();

                cyInstance.on('tap', 'node', (evt) => {
                    const node = evt.target;
                    const href = node.data('href');

                    if (href) {
                        window.location.href = href;  // Navigates to the URL in the same tab
                    }
                });
 
                // Add cursor change on hover
                cyInstance.on('mouseover', 'node', () => {
                    cyInstance.container()!.style.cursor = 'pointer';
                });

                cyInstance.on('mouseout', 'node', () => {
                    cyInstance.container()!.style.cursor = '';
                });
           }
        });

        return {
            cy,
        };
    },
});
</script>

<style scoped>
/* Additional styles can be added here */
</style>